...
3. 实现集成测试：
   ...
   - 多提供商 API 切换测试
   - 缓存和降级策略测试
4. 补充本地可一键运行的测试命令与约定（例如 `uv run python -m pytest`）
5. 设置测试覆盖率目标（至少 70%）
...
**优化方案：**
1. 说明：本阶段不在仓库内落地 CI 工作流文件（例如不创建 `.github/workflows/test.yml`），以下内容作为可选后续路线
2. 完善 CI 流程（GitHub Actions）：
   - 自动运行测试和 lint
...
**优化方案：**
0. 说明：本节讨论的是“在 CI/CD 场景使用 aicmd 的产品能力优化”，不是在本仓库内配置 CI 工作流
1. 非交互模式增强：
   - 更好的脚本集成支持
   - 标准化的退出码
   - 结构化的 JSON/YAML 输出
