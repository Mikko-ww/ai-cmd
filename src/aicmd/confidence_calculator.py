...
class ConfidenceCalculator:
    ...
    def calculate_confidence(self, confirmation_count: int, rejection_count: int,
                           created_at: Optional[str] = None,
                           last_used: Optional[str] = None) -> float:
        """
        基于反馈次数计算置信度
        ...
        if total_feedback == 0:
            ...
            if net_score >= 0:
                ...
                base_confidence = 0.5 * (1 + net_score / (abs(net_score) + total_feedback * 0.5))
    ...
    def _calculate_time_decay_factor(self, created_at: str, last_used: Optional[str] = None) -> float:
        """
        计算时间衰减因子
        ...
        try:
            ...
            # 时间解析失败时不应影响置信度计算
            self.degradation_manager.logger.warning(f"Time decay calculation failed: {e}")
            return 1.0
    
    def update_feedback(self, query_hash: str, command: str, confirmed: bool,
                       similarity_score: Optional[float] = None) -> Tuple[bool, float]:
        """
        更新用户反馈并重新计算置信度
        ...
        def feedback_operation():
            ...
            if not cache_entry:
                # 没有找到缓存条目，可能需要创建新的
                self.degradation_manager.logger.warning(f"No cache entry found for hash: {query_hash}")
                return False, 0.0
            ...
            if update_success:
                # 记录反馈历史
                self._record_feedback_history(query_hash, command, confirmed, similarity_score)
    ...
    def _update_cache_confidence(self, query_hash: str, confirmation_count: int,
                               rejection_count: int, new_confidence: float) -> bool:
        """更新缓存条目的置信度和反馈计数"""
        if not self.cache_manager or not self.cache_manager.db.is_available:
            return False
        ...
        result = self.cache_manager.db.execute_query(
            update_sql, (new_confidence, confirmation_count, rejection_count, query_hash)
        )
        
        return result and isinstance(result, int) and result > 0
    
    def _record_feedback_history(self, query_hash: str, command: str, confirmed: bool,
        ...
        try:
            self.cache_manager.db.execute_query(insert_sql, (query_hash, command, action))
        except Exception as e:
            self.degradation_manager.logger.warning(f"Failed to record feedback history: {e}")
    ...
    def find_exact_match_by_hash(self, query_hash: str) -> Optional[Any]:
        ...
        result = self.cache_manager.db.execute_query(select_sql, (query_hash,), fetch=True)
    ...
    def get_confidence_stats(self) -> Dict[str, Any]:
        ...
        def stats_operation():
            ...
            for range_name, min_conf, max_conf in confidence_ranges:
                ...
                if result and isinstance(result, list) and len(result) > 0:
                    stats[f'{range_name}_confidence_count'] = result[0][0]
                else:
                    stats[f'{range_name}_confidence_count'] = 0
            ...
            if result and isinstance(result, list) and len(result) > 0:
                row = result[0]
                stats['total_confirmations'] = row[0] or 0
                stats['total_rejections'] = row[1] or 0
        ...
        def fallback_operation():
            return {"status": "error", "message": "Confidence stats unavailable"}
        
        return self.degradation_manager.with_cache_fallback(
            stats_operation, fallback_operation, "get_confidence_stats"
        )
    
    def cleanup_low_confidence_entries(self, threshold: float = 0.1, max_cleanup: int = 50):
        """清理低置信度的缓存条目"""
    ...
    def recalculate_all_confidence(self) -> int:
        ...
        def recalc_operation():
            ...
            for row in result:
                ...
                # 更新数据库
                if self._update_cache_confidence(query_hash, conf_count, rej_count, new_confidence):
                    update_count += 1
